...
def test_graduation_date_match(key_details_df):
    ...
    assert grad_date == '08. August 2022', f"Graduation Date mismatch: Expected '08. August 2022', got '{grad_date}'"
...
def test_module_grade_renewables(key_details_df):
    grade = get_detail_value(key_details_df, 'Module', 'Renewables')
    assert grade == 'good (2.0)', f"Renewables Grade mismatch: Expected 'good (2.0)', got '{grade}'"

def test_module_grade_smart_grids(key_details_df):
    grade = get_detail_value(key_details_df, 'Module', 'Smart Grids')
    assert grade == 'good (2.0)', f"Smart Grids Grade mismatch: Expected 'good (2.0)', got '{grade}'"

def test_module_grade_cyber_intelligence(key_details_df):
    grade = get_detail_value(key_details_df, 'Module', 'Cyber Intelligence')
    assert grade == 'good (2.4)', f"Cyber Intelligence Grade mismatch: Expected 'good (2.4)', got '{grade}'"

def test_module_grade_rl(key_details_df):
    grade = get_detail_value(key_details_df, 'Module', 'Reinforcement Learning')
    assert grade == 'good (2.3)', f"Reinforcement Learning Grade mismatch: Expected 'good (2.3)', got '{grade}'"

def test_module_grade_sustainability(key_details_df):
    grade = get_detail_value(key_details_df, 'Module', 'Sustainability')
    assert grade == 'good (1.7)', f"Sustainability Grade mismatch: Expected 'good (1.7)', got '{grade}'"
